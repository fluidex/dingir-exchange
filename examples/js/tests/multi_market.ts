...
async function orderTest() {
  ...
  const openOrders = (await axios.get(`http://${server}/exchange/api/action/orders/all/1`)).data;
  ...
  const closedOrders = (await axios.get(`http://${server}/exchange/api/panel/closedorders/all/1`)).data;
