...
use crate::message::{new_message_receiver_with_kafka_backend, ChannelMessageReceiver};
...
pub struct Controller {
    ...
    pub message_receiver: Rc<RefCell<ChannelMessageReceiver>>,
...
impl Controller {
    pub fn new(settings: config::Settings) -> Controller {
        ...
        let message_receiver = Rc::new(RefCell::new(new_message_receiver_with_kafka_backend(&settings.brokers).unwrap()));
    ...
    fn check_service_available(&self) -> bool {
        ...
        if self.message_receiver.borrow_mut().is_block() {
            log::warn!("message_receiver full");
            return false;
        }
