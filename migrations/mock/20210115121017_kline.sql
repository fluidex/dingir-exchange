...
insert into market_trade select time, 'ETH_USDT', 1, price, amount, price * amount, 'ask'
...
insert into market_trade select time, 'BTC_USDT', 1, price, amount, price * amount, 'ask'
from (select time, random()*200 + 1000 as price, random()*30 as amount 
from generate_series(now() - interval '1 day', now() + interval '1 day', interval '3 s') as time) t;
